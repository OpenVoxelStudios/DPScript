...
use crate::lsp::analysis::selection::position_to_point;
use crate::lsp::analysis::{DefinitionResolver, LEGEND_MODIFIERS, LEGEND_TYPES};
use crate::lsp::analysis::{
    handle_goto_definition, handle_selection_range, handle_semantic_tokens_full_delta,
    handle_semantic_tokens_range,
};
use crate::lsp::config::WorkspaceSettings;
use crate::lsp::document::DocumentStore;
use crate::lsp::language::{DocumentParserPool, LanguageCoordinator};
use crate::lsp::language::{LanguageEvent, LanguageLogLevel};
use crate::lsp::lsp::{SettingsEvent, SettingsEventKind, SettingsSource, load_settings};
...
impl TreeSitterLs {
    ...
    async fn parse_document(
        ...
        if let Some(language_name) = language_name {
            ...
            if let Some(tree) = parsed_tree {
                ...
                self.handle_language_events(&[]).await;
                return;
            }
        }
    ...
    async fn apply_settings(&self, _settings: WorkspaceSettings) {
        self.handle_language_events(&[]).await;
    }
...
impl LanguageServer for TreeSitterLs {
    ...
    async fn semantic_tokens_full(
        ...
        let query = self.language.get_highlight_query();
        ...
        let result = {
            ...
            crate::lsp::analysis::handle_semantic_tokens_full(
                text,
                tree,
                &query,
                Some(&language_name),
    ...
    async fn goto_definition(
        ...
        // Get locals query
        let locals_query = self.language.get_locals_query();
    ...
    async fn code_action(&self, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
        ...
        let highlight_query = self.language.get_highlight_query();
        let locals_query = self.language.get_locals_query();
        let injection_query = self.language.get_injection_query();

        let queries = (highlight_query.as_ref(), Some(locals_query.as_ref()));

        // Use the injection-aware handler with coordinator
        if let Ok(mut pool) = self.parser_pool.lock() {
            Ok(
                crate::lsp::analysis::refactor::handle_code_actions_with_injection_and_coordinator(
                    &uri,
                    text,
                    tree,
                    domain_range,
            ...
            Ok(
                crate::lsp::analysis::refactor::handle_code_actions_with_injection_query(
                    &uri,
                    text,
                    tree,
                    domain_range,
