...
pub struct CodeActionContext<'a> {
    ...
    pub coordinator: Option<&'a crate::lsp::language::LanguageCoordinator>,
    pub parser_pool: Option<&'a mut crate::lsp::language::DocumentParserPool>,
}
...
pub struct InspectTokenParams<'a> {
    ...
    pub offset_from_query: Option<crate::lsp::language::injection::InjectionOffset>,
}
...
impl<'a> InspectTokenParams<'a> {
    ...
    pub fn with_offset(
        mut self,
        offset: Option<crate::lsp::language::injection::InjectionOffset>,
    ) -> Self {
...
pub fn create_inspect_token_action(params: InspectTokenParams) -> CodeActionOrCommand {
        ...
        && hierarchy.len() > 1
    {
        ...
        let offset = params
            .offset_from_query
            .unwrap_or(crate::lsp::language::injection::DEFAULT_OFFSET);
    ...
    if let Some((highlights_query, locals_query)) = params.queries {
        ...
        while let Some(m) = matches.next() {
            ...
            let filtered_captures =
                crate::lsp::language::filter_captures(highlights_query, m, params.text);
        ...
        if let Some(locals) = locals_query {
            ...
            while let Some(m) = matches.next() {
                // Filter captures based on predicates
                let filtered_captures =
                    crate::lsp::language::filter_captures(locals, m, params.text);
                for c in filtered_captures {
                    if c.node == *params.node {
...
fn is_within_effective_range(
    ...
    content_node: &Node,
    position: usize,
    offset: crate::lsp::language::injection::InjectionOffset,
) -> bool {
...
fn create_injection_aware_action(
    ...
    pattern_index: usize,
    cursor_byte: usize,
    coordinator: Option<&crate::lsp::language::LanguageCoordinator>,
    parser_pool: Option<&mut crate::lsp::language::DocumentParserPool>,
    injection_query: Option<&Query>,
) -> CodeActionOrCommand {
    // Parse offset directive for the SPECIFIC pattern that matched
    let offset_from_query = injection_query.and_then(|q| {
        crate::lsp::language::injection::parse_offset_directive_for_pattern(q, pattern_index)
    });
...
fn handle_nested_injection(
    ...
    relative_byte: usize,
    hierarchy: &[String],
    coord: &crate::lsp::language::LanguageCoordinator,
    pool: &mut crate::lsp::language::DocumentParserPool,
    ...
    let inj_query = coord.get_injection_query();
...
pub fn handle_code_actions_with_injection_and_coordinator(
    ...
    capture_context: Option<(&str, &CaptureMappings)>,
    injection_query: Option<&Query>,
    coordinator: &crate::lsp::language::LanguageCoordinator,
    parser_pool: &mut crate::lsp::language::DocumentParserPool,
) -> Option<Vec<CodeActionOrCommand>> {
    let context = CodeActionContext {
        uri,
        text,
...
"#;
        ...
        let coordinator = crate::lsp::language::LanguageCoordinator::new();
  ...
  (#set! injection.language "regex"))
    ...
    fn test_pattern_aware_offset_parsing() {
        ...
        for i in 0..pattern_count {
            let offset =
                crate::lsp::language::injection::parse_offset_directive_for_pattern(&query, i);
            if offset.is_none() {
